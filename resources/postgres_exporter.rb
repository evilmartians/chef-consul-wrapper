...
action :add do
  tags = new_resource.tags +
         ...
         %w[postgres_exporter prometheus]
  ...
  notes = "#{service_type}_#{new_resource.service_name} should listen on #{new_resource.address}:#{new_resource.port}"
  ...
  consul_definition "#{service_type}-#{new_resource.service_name}_http" do
    type 'check'
    parameters(
